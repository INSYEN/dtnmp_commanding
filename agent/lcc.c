...
int lcc_run_ctrl_mid(mid_t *id)
{
    ...
    {
    	DTNMP_DEBUG_INFO("lcc_run_ctrl_mid","Found control (%d).", adm_ctrl->num_parms);
    	if(adm_ctrl->run==NULL)
    	{
    		DTNMP_DEBUG_ERR("licc_run_ctrl_mid","Can't find control gathering function",NULL);
    		return 0;
    	}
