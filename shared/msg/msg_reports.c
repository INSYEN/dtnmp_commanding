...
uint8_t *rpt_serialize_data(rpt_data_t *msg, uint32_t *len)
{
	...
	*len = 0;
	...
	temp_space = (uint8_t**)STAKE(num_rpts * sizeof(uint8_t *));
	temp_len = (uint32_t*)STAKE(num_rpts * sizeof(uint32_t));
		                         ...
		                         uint32_t *bytes_used)
{
	...
	*bytes_used = 0;
	...
	if((result = (rpt_items_t*)STAKE(sizeof(rpt_items_t))) == NULL)
	{
		                       	 ...
		                       	 uint32_t *bytes_used)
{
	...
	*bytes_used = 0;
	...
	/* Step 1: Allocate the new message structure. */
	if((result = (rpt_data_t*)STAKE(sizeof(rpt_data_t))) == NULL)
	{
	...
	for(i = 0; i < num_rpts; i++)
	{
		...
		if((cur_entry = (rpt_data_entry_t*)STAKE(sizeof(rpt_data_entry_t))) == NULL)
		{
			DTNMP_DEBUG_ERR("rpt_deserialize_data","Can't alloc %d bytes.",
		...
		/* Allocate the size. */
		if((cur_entry->contents = (uint8_t*)STAKE(cur_entry->size)) == NULL)
		{
