...
Lyst utils_datacol_copy(Lyst col)
{
	...
	for(elt = lyst_first(col); elt; elt = lyst_next(elt))
	{
		...
		if((new_entry = (datacol_entry_t*)STAKE(sizeof(datacol_entry_t))) == NULL)
		{
		...
		if((new_entry->value = (uint8_t*)STAKE(new_entry->length)) == NULL)
		{
			DTNMP_DEBUG_ERR("utils_datacol_copy","Failed to alloc %d bytes.",
					        new_entry->length);
			MRELEASE(new_entry);
...
Lyst utils_datacol_deserialize(uint8_t* buffer, uint32_t buffer_size, uint32_t *bytes_used)
{
	...
	*bytes_used = 0;
	...
	for(i = 0; i < num; i++)
	{
		...
		/* Make new entry. */
		if((entry = (datacol_entry_t*) STAKE(sizeof(datacol_entry_t))) == NULL)
		{
	...
	*size = num_sdnv.length;
    ...
    for(elt = lyst_first(datacol); elt; elt = lyst_next(elt))
    {
    	...
    	if(entry != NULL)
    	{
    		...
    		DTNMP_DEBUG_WARN("utils_datacol_serialize","Fsize %d", entry->length);
...
/******************************************************************************
 *
 * \par Function Name: utils_lyst_to_args
 *
 * \par Purpose: Parses a lyst object into the variadic arguments provided to
 *               the function
 *
 * \return Number of arguments copied
 *
 *
 * \param[in]   input  A pointer to the lyst
 * \param[in]   count  Number of arguments provided
 * Everything else is variadic, and are treated like pointers
 *
 * \par Notes:
 *
 * Modification History:
 *  MM/DD/YY  AUTHOR         DESCRIPTION
 *  --------  ------------   ---------------------------------------------
 *  02/13/15  J.P Mayer      Initial implementation,
 *****************************************************************************/
uint16_t utils_lyst_to_args(Lyst* input,uint16_t count, ...)
{
	unsigned int countArgs = count > lyst_length(*input) ? count : lyst_length(*input);
	unsigned int x = 0;
	LystElt elt = 0;
	...
	for(elt = lyst_first(*input); x < countArgs; x++, elt = lyst_next(elt))
	{
		void* curArgPtr = va_arg(ap,void*);
		datacol_entry_t* curEntry = ((datacol_entry_t *)lyst_data(elt));
		memcpy(curArgPtr,curEntry->value,curEntry->length);
	}
	va_end(ap);
	return x;
}

inline void* safe_psm_ion_alloc(char* file, int line, size_t size,sem_t* sema)
{
	DTNMP_DEBUG_INFO(file,"Allocating %d %d",line,size);
	/*ionAttach();
	Sdr sdr = getIonsdr();
	if(sdr==NULL)
	{
		DTNMP_DEBUG_ERR("safe_alloc","Can't find SDR",NULL);
		return NULL;
	}
	sdr_begin_xn(sdr);

	void* ptr = allocFromIonMemory(file,line,size);
	sdr_end_xn(sdr);

	DTNMP_DEBUG_INFO(file,"DONE Allocating %d %d",line,size);
	*/
...
void safe_psm_ion_free(char* file, int line, void* ptr,sem_t* sema)
{
	...
	releaseToIonMemory(file,line,ptr);
	sdr_end_xn(sdr);
	*/
	if(ptr!=NULL)
		free(ptr);
