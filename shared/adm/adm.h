...
/** JPM
* Describes the datapassing structure for the to_string function
* This really exists so I don't have to perform a minor refactoring EVERY
* SINGLE TIME I decide to change something
*/
 ...
 * - The string function generates a String representation of the data value. JPM: This actually generates a variable to the queue, stringification (or socket transmission) is done at the UI layer
...
typedef variableQueueEntry* (*adm_string_fn)(uint8_t* buffer, uint64_t buffer_len, uint64_t data_len, uint32_t *str_len, char* admName);
...
//    uint8_t mid_len;             /**> Length, in bytes, of the MID.        */
    uint8_t num_parms;           /**> # params needed to complete this MID.*/
	Lyst param_types;			 /**> expr.h parameter types for validation*/
...
uint8_t		 adm_add_ctrl_parameter_type(uint8_t type,int8_t* mid_str);
