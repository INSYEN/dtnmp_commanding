...
int main(int argc, char *argv[])
{
	...
	/*Start memory manager semaphore*/
	memSema = sem_open("/dtnmp",O_CREAT);
...
#ifdef HAVE_NETUI
	 if(pthread_create(&ui_thr, NULL, net_ui_thread, (void *)ui_thr_name))
    {
        DTNMP_DEBUG_ERR("main","Can't create pthread %s, errnor = %s",
        		        ui_thr_name, strerror(errno));
        exit(EXIT_FAILURE);
